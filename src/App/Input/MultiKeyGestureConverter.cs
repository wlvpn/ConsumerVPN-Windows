        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                foreach (var sequence in sequences)
                {
                    ...
                    string temp;
                    while ((temp = keyStrings[modifiersCount]) != null && TryGetModifierKeys(temp.Trim(), out ModifierKeys currentModifier))
                    {
                        modifiersCount++;
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (value is MultiKeyGesture gesture)
                {
                    ...
                    for (var i = 0; i < gesture.KeySequences.Count; i++)
                    {
                        if (i > 0)
                            builder.Append(", ");
                        ...
                        for (var j = 1; j < sequence.Keys.Count; j++)
                        {
                            builder.Append("+");
                            builder.Append((string)keyConverter.ConvertTo(context, culture, sequence.Keys[0], destinationType));
                        }
