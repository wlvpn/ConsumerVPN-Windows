...
namespace WLVPN
{
    public class AppBootstrapper : BootstrapperBase
    {
        ...
        public AppBootstrapper()
        {
            ...
            // Automatically save settings on property change
            Properties.Settings.Default.PropertyChanged += (s, e) =>
            {
                try
                {
                    _debounceTimer.Debounce(500, (_) => Properties.Settings.Default.Save());
                }
                catch (Exception ex)
                {
                    Log.Error(ex, $"Failed to save the settings. Settings property '{e.PropertyName}' changed.");
                }
            };
